...
from typing import TYPE_CHECKING, Iterable, List, Optional, Tuple, Dict, Union, Literal
...
class RezkaStream:
    ...
    async def _get_cache(self, url: str, form_data: str):
        if not aiorezka.use_cache:
            return None
        key = f"{url}?{form_data}"
        return await self.api_client.cache.get(key)

    async def _request_cdn_series(self, movie_id: int, audio_track_id: int, season: int, episode: int):
        ...
        if resp:
            self.logger.debug(
                ...
                movie_id, audio_track_id, season, episode
            )
            return resp

        self.logger.debug(
            "Getting series stream for movie_id=%s, audio_track_id=%s, season=%s, episode=%s\nURL: %s",
            movie_id, audio_track_id, season, episode, url
        )
    ...
    async def get_episodes(self, movie_id: int, audio_track_id: int, season: int) -> List[int]:
        ...
        if not episodes_html:
            return []
        return [int(x.get('data-episode_id')) for x in
                BeautifulSoup(episodes_html, 'html.parser').find_all('li', class_='b-simple_episode__item')]

    async def get_series_stream(
        self, movie_id: int, audio_track_id: int, season: int, episode: int
